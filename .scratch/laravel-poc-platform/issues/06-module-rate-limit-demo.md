...
- [x] A demo route is gated by a custom rate-limiting middleware (a dedicated limiter, not the app's global default) with a low, demo-friendly threshold (e.g. 5 requests/10s)
- [x] The demo route checks its own flag (`Feature::active($slug)`) before running, returning a clear non-200 "unavailable" response when the flag is off — independent of the throttle's own 429
- [x] A Pest HTTP feature test hits the demo route repeatedly with the flag on and asserts the request under the limit succeeds while the request past the limit returns 429
