...
- [x] A demo route triggers a notification (Laravel's notification system, broadcast channel) on a channel distinct from any other real-time module's channel
- [x] The demo page subscribes via Echo and renders incoming notifications live in a bell icon + feed UI, unread count included
...
- [x] A Pest feature test triggers a notification with the flag on and asserts it's broadcast on the module's own channel (via Laravel's notification/broadcast assertion helpers)
